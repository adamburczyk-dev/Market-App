...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        # Ensure the source streams too, so subscription works regardless of start order.
        await ensure_stream(js, settings.NATS_SOURCE_STREAM, ["market_data.>"])
        await ensure_stream(js, settings.NATS_FUNDAMENTALS_STREAM, ["fundamentals.>"])
        await ensure_stream(js, settings.NATS_COMPANY_STREAM, ["company.>"])
    ...
    if nats_client is not None:
        try:
            ...
            await market_sub.start()
            fundamentals_sub = EventSubscriber(
                nats_client.jetstream(),
                settings.NATS_FUNDAMENTALS_SUBJECT,
                settings.NATS_FUNDAMENTALS_DURABLE,
                service.handle_fundamentals_event,
                max_deliver=settings.NATS_MAX_DELIVER,
            )
            await fundamentals_sub.start()
            company_sub = EventSubscriber(
                nats_client.jetstream(),
                settings.NATS_COMPANY_SUBJECT,
                settings.NATS_COMPANY_DURABLE,
                service.handle_company_classified_event,
                max_deliver=settings.NATS_MAX_DELIVER,
            )
            await company_sub.start()
            subscribers = [market_sub, fundamentals_sub, company_sub]
        except Exception as exc:  # noqa: BLE001
