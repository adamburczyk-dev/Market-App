...
async def test_compute_stores_and_publishes():
    ...
    service = FeatureEngineService(
        FakeMarketDataClient(n=30), FeatureStore(), publisher, min_bars=20
    )

    fv = await service.compute_for_symbol("AAPL", Interval.D1)
    assert fv is not None
    assert service.get_features("AAPL", Interval.D1) is fv
...
async def test_insufficient_bars_returns_none_and_no_event():
    publisher = NullPublisher()
    service = FeatureEngineService(
        FakeMarketDataClient(n=5), FeatureStore(), publisher, min_bars=20
    )
    fv = await service.compute_for_symbol("AAPL", Interval.D1)
    assert fv is None
...
async def test_handle_market_data_event_triggers_compute():
    ...
    assert service.get_features("MSFT", Interval.D1) is not None
    assert service.list_symbols() == ["MSFT"]
    assert len(publisher.published) == 1
