...
class CapacityProbe:
    n_rows: int
    n_features: int
    auc_train_real: float
    auc_train_shuffled: float
...
def run_capacity_probe(
    ...
    an ops-call budget; the comparison is like-for-like because both fits see
    ...
    real = _fit_and_score(x, y, over)

    rng = np.random.default_rng(over.seed)
    shuffled_labels = y.copy()
    rng.shuffle(shuffled_labels)
    shuffled = _fit_and_score(x, shuffled_labels, over, seed_offset=1)

    production = _fit_and_score(x, y, production_config or TrainConfig(), seed_offset=2)

    gap = real - shuffled
    if gap >= gap_threshold:
        verdict = (
            "learnable structure EXISTS — the big model separates real labels well beyond "
            "what it memorizes from shuffled ones. A flat production train AUC is then an "
            "optimization/regularization problem (capacity, lr, dropout, early stopping), "
